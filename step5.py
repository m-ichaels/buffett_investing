...
def main():
    # Load tickers from the ROTC filtered results in the Excel file
    file_path = os.path.join('Results', '2022.xlsx')
    try:
        df = pd.read_excel(file_path, sheet_name='ROTC_Filtered_Stocks')
        if 'Ticker' not in df.columns:
            raise ValueError("ROTC_Filtered_Stocks worksheet must contain a 'Ticker' column")
    ...
    if results:
        ...
        with pd.ExcelWriter(file_path, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
            results_df.to_excel(writer, sheet_name='FCF_Filtered_Stocks', index=False)
        
        print(f"\nFCF filtering complete! {len(results)} stocks meet the criterion.")
        print(f"Results saved to '{file_path}' in 'FCF_Filtered_Stocks' worksheet")
